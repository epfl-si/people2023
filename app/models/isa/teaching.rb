...
class Isa::Teaching
  ...
  def secondary_teaching
    @sec_tea ||= if @ta.present? and @ta['enseignementSecSecondondaire'].present?
  ...
  def doctoral_teaching
    @sec_tea ||= if @ta.present? and @ta['directorThese'].present?
  ...
  def phd_directorships
    @phd_dir ||= if @ta.present? and @ta['directorThese'].present?
                   @ta['directorThese'].select do |e|
                     ee = e['dateFinValidite']
                     !(ee.present? && Date.parse(ee) < Date.today)
                   end
